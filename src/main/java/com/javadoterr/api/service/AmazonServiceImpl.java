...
public class AmazonServiceImpl implements AmazonService{
	...
	private void initializeAmazon() {
		this.s3Client = new AmazonS3Client(new BasicAWSCredentials(ConstantUtils.ACCESS_KEY, ConstantUtils.SECRTE_KEY));
	}
	...
	public String uploadFile(MultipartFile multipartFile, User user) {
		...
		try {
			...
			String fileName = new Date().getTime()+"-"+multipartFile.getOriginalFilename().replace(" ", "-");
			fileUrl = ConstantUtils.END_POINT_URL+"/"+ConstantUtils.BUCKET_NAME+"/"+fileName;
			
			s3Client.putObject(new PutObjectRequest(ConstantUtils.BUCKET_NAME,
			...
			jsonObject.put("imageUrl", fileName);
